...
NOTE: The `audit-immutability.sql` file can remain in the repo as documentation, but it is no longer needed for setup. Do not delete it — it serves as a reference.
  ...
  <verify>
Verify the migration directory exists: `ls backend/prisma/migrations/*audit_immutability*`. Verify the migration SQL contains the trigger functions: `grep "prevent_audit_update" backend/prisma/migrations/*audit_immutability*/migration.sql`. Verify db:setup no longer has psql: `grep "psql" backend/package.json` should return no matches. Confirm the trigger infrastructure exists in the database using pg_catalog queries (these confirm existence regardless of table contents):
```sql
SELECT proname FROM pg_proc WHERE proname = 'prevent_audit_update';
SELECT proname FROM pg_proc WHERE proname = 'prevent_audit_delete';
SELECT tgname FROM pg_trigger WHERE tgname = 'audit_no_update';
SELECT tgname FROM pg_trigger WHERE tgname = 'audit_no_delete';
```
Run via: `cd /Users/mmaudet/work/validly/backend && npx prisma db execute --stdin <<< "SELECT proname FROM pg_proc WHERE proname IN ('prevent_audit_update','prevent_audit_delete');"` — should return 2 rows. Run `npx prisma db execute --stdin <<< "SELECT tgname FROM pg_trigger WHERE tgname IN ('audit_no_update','audit_no_delete');"` — should return 2 rows.
  </verify>
  <done>
The audit immutability triggers are in a proper Prisma migration that runs automatically with `prisma migrate deploy`. The db:setup script no longer requires a manual psql step. Fresh environments, CI, and staging deployments get the INSERT-only enforcement automatically. The pg_catalog queries confirm both trigger functions and trigger definitions exist in the database.
  </done>
</task>
...
7. `cd /Users/mmaudet/work/validly/backend && npx prisma db execute --stdin <<< "SELECT proname FROM pg_proc WHERE proname IN ('prevent_audit_update','prevent_audit_delete');"` — 2 rows returned
8. `cd /Users/mmaudet/work/validly/backend && npx prisma db execute --stdin <<< "SELECT tgname FROM pg_trigger WHERE tgname IN ('audit_no_update','audit_no_delete');"` — 2 rows returned
</verification>
...
- pg_catalog confirms both trigger functions and both trigger definitions exist in the database
</success_criteria>
