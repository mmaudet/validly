            ...
            <div key={phase.id} className="flex items-start flex-1 min-w-0">
              ...
              <div className="flex flex-col items-center flex-shrink-0">
                <button
                  ...
                  className={`h-10 w-10 rounded-full flex items-center justify-center text-sm font-semibold transition-all ${circleColor} ${isActive ? 'ring-2 ring-blue-500 ring-offset-2' : ''}`}
                  title={phase.name}
                >
                  {phase.order}
                </button>
                <span className="mt-1 text-xs text-center text-gray-600 truncate max-w-[100px]">
                  {phase.name}
                </span>
              </div>
            </div>
