...
export const emailService = {
  ...
  async sendPasswordReset(input: PasswordResetEmailInput) {
    ...
    const subject = t('email.password_reset_subject', { appName: 'Validly' });

    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #1a1a1a;">
          ${t('email.password_reset_heading')}
        </h2>
        <p style="color: #444; line-height: 1.6;">
          ${t('email.password_reset_body')}
        </p>
        <div style="margin: 30px 0; text-align: center;">
          <a href="${input.resetUrl}" style="display: inline-block; padding: 12px 32px; background-color: #2563eb; color: white; text-decoration: none; border-radius: 6px; font-weight: bold;">
            ${t('email.password_reset_button')}
          </a>
        </div>
        <p style="color: #888; font-size: 12px;">
          ${t('email.password_reset_expiry')} ${t('email.password_reset_ignore')}
        </p>
      </div>
    `;
